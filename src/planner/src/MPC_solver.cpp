...
void MPCPathFinder::GetRefPath(std::vector<Eigen::Vector3d> jps_nodes)
{
    ...
    for (int i = 0; i < int(jps_nodes.size() - 1); ++i)
    {
        ...
        for (int j = 0; j < num_samples; ++j)
        {
            ...
            if (!ref_nodes.empty())
            {
                if (coord.isApprox(ref_nodes.back()))
                {
                    continue;
                }
                else
    ...
    if (!jps_nodes.back().isApprox(ref_nodes.back()))
    {
...
void MPCPathFinder::MPCSolver()
{
    if (!check_arrived(pos, goal) && fail <= 1 * N)
    {
        ...
        try
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                model.addConstr(posNvar[0][j] == pos[j] + h * v[j], "c" + std::to_string(cc++));
                model.addConstr(vNvar[0][j] == v[j] + h * a[j], "c" + std::to_string(cc++));
                model.addConstr(aNvar[0][j] == a[j] + h * jerk[j], "c" + std::to_string(cc++));
            }
            // ROS_INFO("MODEL 1: C = %d", cc);
            // N > 1
            for (int i = 1; i < N; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    model.addConstr(posNvar[i][j] == posNvar[i - 1][j] + h * vNvar[i - 1][j], "c" + std::to_string(cc++));
                    model.addConstr(vNvar[i][j] == vNvar[i - 1][j] + h * aNvar[i - 1][j], "c" + std::to_string(cc++));
                    model.addConstr(aNvar[i][j] == aNvar[i - 1][j] + h * jerkNvar[i - 1][j], "c" + std::to_string(cc++));
                }
            }
            // ROS_INFO("MODEL 2: C = %d", cc);

            // Add Constrains 2
            // SC: Ai·pk<ci; Ai·pk+1<ci;
            ...
            for (int i = 0; i < N - 1; i++)
            {
                ...
                // for (int j = 0; j < scNum; j++)
                for (int j = sc_idx - 1 + i; j <= sc_idx + 3 + i; j++)
                {
                    if (j < 0)
                        j = 0;
                    ...
                    for (int k = 0; k < bVec[j].size(); k++)
                    {
                        model.addGenConstrIndicator(m[i][j], 1,
                                                    posNvar[i][0] * AVec[j](k, 0) + posNvar[i][1] * AVec[j](k, 1) + posNvar[i][2] * AVec[j](k, 2) <= bVec[j](k), "c" + std::to_string(cc++));
                        model.addGenConstrIndicator(m[i][j], 1,
                                                    posNvar[i + 1][0] * AVec[j](k, 0) + posNvar[i + 1][1] * AVec[j](k, 1) + posNvar[i + 1][2] * AVec[j](k, 2) <= bVec[j](k), "c" + std::to_string(cc++));
                    }
                    expr += m[i][j];
                }
